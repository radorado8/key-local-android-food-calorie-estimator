...
import { MACRO_COLORS, MACRO_KEYS, resolveMacroGoals, validMacroGoals } from '../utils/macroGoals';
    ...
    <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={[styles.card, { backgroundColor: colors.modalBg }]}>
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
          ...
          {error && <Text accessibilityRole="alert" style={{ color: '#EF4444' }}>{t.macroGoalsInvalid}</Text>}
          <View style={styles.actions}>
            ...
            <Pressable onPress={save} style={[styles.button, { backgroundColor: colors.accent }]}><Text style={[styles.buttonLabel, { color: colors.bg }]}>{t.save}</Text></Pressable>
          </View>
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 24, maxHeight: '90%', width: '100%', maxWidth: 480, alignSelf: 'center', overflow: 'hidden' },
